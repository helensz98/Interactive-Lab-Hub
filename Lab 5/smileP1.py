...
if(len(sys.argv)>1):
   try:
      ...
      print("Failed to load the image are you sure that:", sys.argv[1],"is a pa$
else:
   try:
...
while(True):
   ...
   if (not users):
      state = 0
      pre = 0
      cv2.putText(img, "No user detected", (50, 50), cv2.FONT_HERSHEY_COMPLEX,1$
#      if(state != pre):
#         os.system("mpg321 no_user.mp3")
   elif (not smiling):
      state = 1
      pre = 1
      cv2.putText(img, "Say cheese ------", (50, 50), cv2.FONT_HERSHEY_COMPLEX,1,(2$
#      if(state != pre):
#         os.system("mpg321 cheese.mp3")
#         pre = state
   else:
      state = 2
      cv2.putText(img, "Nice smile!", (50, 50), cv2.FONT_HERSHEY_COMPLEX,1,(255,255$
      if(state != pre):

...
button = qwiic_button.QwiicButton(0x6F)
button.begin()
...
while(True):
  ...
  if (button.is_button_pressed()):
     ...
     if(nSmile < nFace):
        ...
        os.system("mpg321 t2.mp3")
        selfie = False

  if (not users):
     cv2.putText(img, "No user detected", (50, 50), cv2.FONT_HERSHEY_COMPLEX,1,(255,255,255),2)
     selfie = False

  elif (selfie):
     #this is the silence file so that words are not cut off
     os.system("mpg321 out.mp3")
     os.system("mpg321 t3.mp3")
     r, img1 = cap.read()
     cv2.imwrite('image'+str(len(used))+".png",img1)
     used.append(1)
     selfie = False
